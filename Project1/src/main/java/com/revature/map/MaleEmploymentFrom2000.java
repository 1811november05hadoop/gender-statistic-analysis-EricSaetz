...
/**
 * Maps male Employment Rate of each country using the earliest recorded data point(minimum 2000)
...
public class MaleEmploymentFrom2000 extends Mapper<LongWritable, Text, Text, DoubleWritable> {
		...
		throws IOException, InterruptedException {
			...
			if (data.length>=60) {
				...
				category = data[2];
					...
					if (category.contains("Employment to population ratio") && category.contains(" male (%)")) {
						for (int i=44;i<=60;i++) {
							if (!data[i].equals("")) {
